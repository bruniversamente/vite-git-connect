...
export default function HeaderDropdownBlog() {
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className="absolute inset-x-0 top-16 bg-background/80 backdrop-blur border-b border-border shadow-sm"
    >
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({length:8}).map((_,i)=>(
            <Link key={i} to={`/blog/post-${i+1}`} className="block p-4 rounded-lg bg-muted hover:bg-muted/80 min-h-24">
