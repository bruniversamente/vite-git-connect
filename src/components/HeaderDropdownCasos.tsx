      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({length:8}).map((_,i)=>(
            <Link key={i} to={`/casosdesucesso/caso-${i+1}`} className="block p-4 rounded-lg bg-neutral-50 hover:bg-neutral-100 min-h-24">
              <div className="font-medium">Projeto {i+1}</div>
            </Link>
          ))}
        </div>
      </div>
